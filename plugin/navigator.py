...
This plugin has been created using the pyang tree plugin.
It gathers data from a yang file and generates a data.txt
tha can be used for purposes, such as validating RPCs or
JSONs (following RFC 7951) or autogenerating code or CLIs.
...
def pyang_plugin_init():
    plugin.register_plugin(navigatorPlugin())

class navigatorPlugin(plugin.PyangPlugin):
    def __init__(self):
        plugin.PyangPlugin.__init__(self, 'navigator')
    ...
    def add_opts(self, optparser):
        optlist = [
            optparse.make_option("--navigator-help",
                                 dest="tree_help",
                                 action="store_true",
                                 help="Print help on tree symbols and exit"),
            optparse.make_option("--navigator-depth",
                                 type="int",
                                 dest="tree_depth",
                                 help="Number of levels to print"),
            optparse.make_option("--navigator-xml",
                                 type="string",
                                 dest="xmlname",
                                 help="Number of levels to print"),
            optparse.make_option("--navigator-line-length",
                                 type="int",
                                 dest="tree_line_length",
                                 help="Maximum line length"),
            optparse.make_option("--navigator-path",
                                 dest="tree_path",
                                 help="Subtree to print"),
            optparse.make_option("--navigator-print-groupings",
                                 dest="tree_print_groupings",
                                 action="store_true",
                                 help="Print groupings"),
            optparse.make_option("--navigator-operation",
                                 type="string",
                                 dest="operation",
                                 help="Print groupings"),
            optparse.make_option("--navigator-no-expand-uses",
        ...
        if plugin.is_plugin_registered('restconf'):
            optlist.append(
                optparse.make_option("--navigator-print-yang-data",
    ...
    def emit(self, ctx, modules, fd):
        ...
        emit_tree(ctx, modules, fd, ctx.opts.tree_depth,
                  ctx.opts.tree_line_length, path, allnodes)
        import json
        # Remove the commented line below to show the whole tree used for the verification.
        #print json.dumps(allnodes)
        '''import sys
        sys.path.insert(0, '/home/tid/')
        from prompt_navigator import prompt_pyang
        prompt_pyang(allnodes)'''

        file = open("data.txt","w")
        file.write(json.dumps(allnodes))
        file.close()
...
def emit_tree(ctx, modules, fd, depth, llen, path, allnodes):
    for module in modules:
        ...
        if len(chs) > 0:
            ...
            print_children(chs, module, fd, '', chpath, 'data', depth, llen,
                           ctx.opts.tree_no_expand_uses, allnodes)
        ...
        for m in mods:
            ...
            for augment in m.search('augment'):
                if (hasattr(augment.i_target_node, 'i_module') and
                    ...
                    print "Augment found, the model is not complete: "+str(augment.arg)
...
def print_children(i_children, module, fd, prefix, path, mode, depth,
                   llen, no_expand_uses, allnodes, width=0):
    if depth == 0:
...
def print_node(s, module, fd, prefix, path, mode, depth, llen,
    ...
    for a in dir(s):
        #print a
	try:
            auxi[a]=str(getattr(s, a))
        except: pass
    import json
    #print json.dumps(auxi)
    pref=None
    try: pref=str(s.i_module.i_prefix)
    except: pass
    #print "attribute error"
    #auxtxt=', '.join("%s: %s" % item for item in attrs.items())
    camino="/"+name
    aux=s.parent
    while aux!=None:
        #print aux.arg
        camino="/"+aux.arg+camino
        aux=aux.parent

    auxname = "/".join(aux2 for aux2 in camino.split("/")[2:])

    desc=""
    key=None

    '''print "Module: "+name
    print "Parent: "+s.parent.arg
    print "Flags: "+flags
    print "Type: "+s.keyword
    print "Config: "+str(s.i_config)
    print "i_orig_module: "+s.i_orig_module.arg
    if hasattr(s, 'i_leafref') and s.i_leafref != None: print "Path: "+str(s.i_leafref.path_.arg)'''
    hijos='["'
    import json
    ...
    try:
        desc=get_desc(s)
    except: desc="No description available"
    try: key=get_key(s)
    except: desc="No key for this element"
    allnodes[auxname]={"name":name,"parent":s.parent.arg,"flags":flags,"type":s.keyword,"orig":s.i_orig_module.arg,"config":s.i_config,"desc":desc,"key":key,"prefix":pref}
    if s.keyword in "leaf-list":
        allnodes[auxname]["itype"]=get_typename(s)

    #   CHILDS
    if hijos != '["':
        allnodes[auxname]["children"]=json.loads(hijos[:-2]+"]")

    #   MANDATORY
    test=s.search_one('mandatory')
    if test != None:
        allnodes[auxname]["mandatory"]=test.arg

    #   MAX-MIN-ELEMENTS
    if s.keyword in ['list','leaf-list']:
        max=s.search_one("max-elements")
        min=s.search_one("min-elements")
        if max!=None: allnodes[auxname]['max-elements']=int(max.arg)
        if min!=None: allnodes[auxname]['min-elements']=int(min.arg)
    ...
    if hasattr(s, 'i_children') and s.keyword != 'uses':
        ...
        if s.keyword in ['choice', 'case']:
            print_children(chs, module, fd, prefix, path, mode, depth,
                           llen, no_expand_uses,allnodes, width - 3)
        else:
            print_children(chs, module, fd, prefix, path, mode, depth, llen,
                           no_expand_uses, allnodes)
